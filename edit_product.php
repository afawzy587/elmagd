<?php
    ...
    if($login->doCheck() == false)
    {
		...
		}else
		{
			...
			{
				$mId =intval($_GET['id']);
				$u = $setting_product->getSettings_productsInformation($mId);
				if($_POST)
				{
					$_product['products_sn']                        =       $mId;
					$_product['products_name']                      =       sanitize($_POST["products_name"]);
					$_product['products_description']               =       sanitize($_POST["products_description"]);

					$edit = $setting_product->setSettings_productsInformation($_product);
					if($edit == 1)
					{
						$logs->addLog(NULL,
							array(
							...
							header("Location:./settings_products.php?action=edit");
							exit;
					}
				}
			}else{
        ...
        <div class="row centerContent">
            <div class="col">
                <form  method="post" id="companyDetailsForm" enctype="multipart/form-data">
					...
					?>
                     <h5><?php echo $lang['SETTINGS_P_EDIT_PRODUCTS'];?></h5>
                        ...
                        <div class="row">
                            <div class="col-md-5">
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-5">
                                        <input type="text" class="form-control" name="products_name" placeholder="<?php echo $lang['SETTINGS_P_NAME'];?>" value="<?php if($_product){echo $_product['products_name'];}else{echo $u['products_name'];}?>">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-5">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="products_description" placeholder="<?php echo $lang['SETTINGS_P_DESCRIPTION_HOLDER'];?>" value="<?php if($_product){echo $_product['products_description'];}else{echo $u['products_description'];}?>">
                                </div>
                            </div>
                        </div>

...
//			$total       = $setting_supplier->getTotalSettings_suppliers($q);
//			$pager->doAnalysisPager("page",$page,$basicLimit,$total,"settings_suppliers.php".$search.$paginationAddons,$paginationDialm);
//			$thispage    = $pager->getPage();
//			$limitmequry = " LIMIT ".($thispage-1) * $basicLimit .",". $basicLimit;
//			$pager       = $pager->getAnalysis();
			$suppliers   = $setting_supplier->getsiteSettings_suppliers($limitmequry,$q);
			$logs->addLog(NULL,
					array(
						"type" 		        => 	"admin",
						"module" 	        => 	"suppliers",
 ...
 <!-- page content -->
    <div class="container mainPageContainer">
        ...
        <div class="row d-flex justify-content-end">
            <div class="col-md-3">
                <form id="supplierSearchForm" action="./settings_suppliers.php">
                    <div class="form-group has-search">
                        ...
                        <input type="text" class="form-control" placeholder="" id="supplierSearch" name="q" value="<?php echo $q;?>">
                    </div>
                </form>
            </div>
        </div>
        ...
        <!-- table row -->
        <div class="row">
            <input type="hidden" value="settings_suppliers" id="table">
            <input type="hidden" value="setting_suppliers" id="permission">
            <div class="col">
               <?php 
					if($_GET['action'] == 'edit'){
						echo alert_message("success",$lang['SETTINGS_SU_EDIT_SUCCESS']);
					}
				?>
                <table class="table table-fluid " id="supplierTable">
                   <?php 
					if(empty($suppliers))
					{
						...
						<thead>
							<tr>
								<th>'.$lang['SETTINGS_SU_NAME'].'</th>
								<th>'.$lang['SETTINGS_SU_PHONE'].'</th>
						...
						{
//							;if($u['suppliers_phone_two']){echo'<br>'.$u['suppliers_phone_two'];}echo
						 echo'<tr id=tr_'.$u['suppliers_sn'].'>
								<td>'.$u['suppliers_name'].'</td>
								<td>'.$u['suppliers_phone_one'].'</td>
								<td class="text-center tableActions">
									<a href="./edit_supplier.php?id='.$u['suppliers_sn'].'"><i class="fas fa-edit mr-3 green" title="'.$lang['EDIT'].'"></i></a>
									<i class="delete fas fa-trash rose" title="'.$lang['DELETE'].'" id="item_'.$u['suppliers_sn'].'"></i>
								</td>
							</tr>';
						}
						echo '</tbody>';

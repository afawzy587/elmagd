...
class systemExpenses
{
	...
	function getsiteExpenses($addon = "",$q="")
	{
		if($q != "")
		{
			$search = "AND `expenses_cheque_sn` LIKE '%".$q."%' ";
		}else{
			$search = "";
		}
		$query = $GLOBALS['db']->query("SELECT * FROM `".$this->tableName."` WHERE `expenses_status` != '0' ".$search." ORDER BY `expenses_sn`  DESC ".$addon);
	...
	function setExpensesInformation($Expenses)
	{
        ...
        if($queryTotal == 1)
        {
			...
			}else
			{
				...
				VALUES
				(NULL , '".$Expenses['expenses_bank_id']."' , '".$Expenses['expenses_bank_account_type']."', '".$id."', '".$open."', '".$Expenses_new."',1)
				");
			}
	...
	function addExpenses($Expenses)
	{
		...
		(`expenses_sn`, `expenses_date`, `expenses_type`, `expenses_amount`, `expenses_cheque_date`, `expenses_cheque_sn`, `expenses_bank_id`, `expenses_bank_account_type`, `expenses_bank_account_id`, `expenses_title`, `expenses_status`) 
		VALUES ( NULL ,'".$Expenses['expenses_date']."','".$Expenses['expenses_type']."','".$Expenses['expenses_amount']."','".$Expenses['expenses_cheque_date']."','".$Expenses['expenses_cheque_sn']."','".$Expenses['expenses_bank_id']."','".$Expenses['expenses_bank_account_type']."','".$Expenses['expenses_bank_account_id']."','".$Expenses['expenses_title']."',1)");

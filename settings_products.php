...
//			$total       = $setting_product->getTotalSettings_products($q);
//			$pager->doAnalysisPager("page",$page,$basicLimit,$total,"settings_products.php".$search.$paginationAddons,$paginationDialm);
//			$thispage    = $pager->getPage();
//			$limitmequry = " LIMIT ".($thispage-1) * $basicLimit .",". $basicLimit;
//			$pager       = $pager->getAnalysis();
			$products   = $setting_product->getsiteSettings_products($limitmequry,$q);
			$logs->addLog(NULL,
					array(
						"type" 		        => 	"admin",
						"module" 	        => 	"products",
 ...
 <!-- page content -->
    <div class="container mainPageContainer">
        ...
        <div class="row d-flex justify-content-end">
            <div class="col-md-3">
                <form id="productSearchForm" action="./settings_products.php">
                    <div class="form-group has-search">
                        ...
                        <input type="text" class="form-control" placeholder="" id="productSearch" name="q" value="<?php echo $q;?>">
                    </div>
                </form>
            </div>
        </div>
        ...
        <!-- table row -->
        <div class="row">
            <input type="hidden" value="settings_products" id="table">
            <input type="hidden" value="settings_product" id="permission">
            <div class="col">
               <?php 
					if($_GET['action'] == 'edit'){
				...
				?>
                <table class="table table-fluid " id="productTable">
                   <?php 
					if(empty($products))
					{
						...
						{
						 echo'<tr id=tr_'.$u['products_sn'].'>
								<td>'.$u['products_name'].'</td>
								<td>'.$u['products_description'].'</td>
								<td class="text-center tableActions">
									<a href="./edit_product.php?id='.$u['products_sn'].'"><i class="fas fa-edit mr-3 green" title="'.$lang['EDIT'].'"></i></a>
									<i class="delete fas fa-trash rose" title="'.$lang['DELETE'].'" id="item_'.$u['products_sn'].'"></i>
								</td>
							</tr>';
						}
						echo '</tbody>';

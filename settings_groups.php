        ...
        <div class="row mt-5">
            <div class="col">
                <p class="blueSky">
                    ...
                    <span class="blueSky"><strong> &gt; </strong>   <?php echo $lang['Settings_user_group'];?></span>
                ...
                <table class="table table-fluid " id="departmentTable">
                   <?php
					if(empty($user_group))
					{
						...
						{
						 echo'<tr id=tr_'.$u['user_group_sn'].'>
								<td>'.$u['user_group_name'].'</td>
								<td>'.$u['user_group_description'].'</td>
								<td class="text-center tableActions">
									<a href="./edit_department.php?id='.$u['user_group_sn'].'"><i class="fas fa-edit mr-3 green" title="'.$lang['SETTINGS_D_EDIT_DEPARMENTS'].'"></i></a>
									<i class="delete fas fa-trash rose" title="'.$lang['DELETE'].'" id="item_'.$u['user_group_sn'].'"></i>
								</td>
							</tr>';
						}
						echo '</tbody>';

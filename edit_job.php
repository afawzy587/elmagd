<?php
    ...
    if($login->doCheck() == false)
    {
		...
		}else
		{
			...
			{
				$mId =intval($_GET['id']);
				$u = $setting_job->getSettings_jobsInformation($mId);
				...
				if($_POST)
				{
					...
					$edit = $setting_job->setSettings_jobsInformation($_job);
					if($edit == 1)
					{
						$logs->addLog(NULL,
							array(
							...
							header("Location:./settings_jobs.php?action=edit");
							exit;
					}
				}
			}else{
        ...
        <div class="row centerContent">
            <div class="col">
                <form  method="post" id="companyDetailsForm" enctype="multipart/form-data">
					...
					?>
                     <h5><?php echo $lang['SETTINGS_J_EDIT_JOBS'];?></h5>
                        ...
                        <div class="row">
                            <div class="col-md-5">
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-5">
                                        <input type="text" class="form-control" name="jobs_name" placeholder="<?php echo $lang['SETTINGS_D_NAME'];?>" value="<?php if($_job){echo $_job['jobs_name'];}else{echo $u['jobs_name'];}?>">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-5">
                                            ...
                                            <select name="jobs_department" class="form-control" id="slct">
                                                ...
                                                <?php
													if($departments)
													{
														...
														{
															echo '<option value="'.$d['departments_sn'].'"';if($_job['jobs_department'] == $d['departments_sn']){echo 'selected';}elseif($u['jobs_department'] == $d['departments_sn']){echo 'selected';}echo'>'.$d['departments_name'].'</option>';
